...
def ensure_users_tier_column(engine):
    """Ensure users.tier exists (Postgres/SQLite). Safe to call on every startup."""
    try:
        dialect = (engine.dialect.name or "").lower()
        if dialect.startswith("postgres"):
            with engine.begin() as conn:
                conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS tier VARCHAR(20) DEFAULT 'free';"))
        elif dialect.startswith("sqlite"):
            with engine.begin() as conn:
        ...
        print(f"Warning: could not ensure tier column on users: {e}")
...
def tier_at_least(user_tier: str, minimum: str) -> bool:
    return TIER_ORDER.get(user_tier or "free", 0) >= TIER_ORDER.get(minimum, 0)

def has_endpoint(name: str) -> bool:
    """True if a Flask endpoint exists (prevents Jinja url_for crashes)."""
    try:
        return name in app.view_functions
    except Exception:
        return False

def can_access(feature_min_tier: str) -> bool:
    """Template-friendly: does current user meet minimum tier?"""
    u = get_current_user()
    if not u:
        return False
    ut = getattr(u, "tier", "free") or "free"
    return tier_at_least(ut, feature_min_tier)


@app.route("/pricing")
def pricing():
    # Simple placeholder pricing page (create templates/pricing.html if desired)
