...
Production-safe seeding: adds missing peptides, skips existing.

- Uses Config.DATABASE_URL (Render/Postgres friendly)
- Idempotent: safe to run multiple times
"""

from models import get_session, AdministrationRoute, StorageMethod, create_database
from database import PeptideDB
from config import Config


def seed_common_peptides(session):
    """Add peptides to the database (idempotent)."""
    ...
    peptides_data = [
{
    ...
    # Additional peptides (from add_new_peptides.py)
    additional_peptides = [
# Weight Loss GLP-1 Agonists
    ...
    combined = peptides_data + additional_peptides
    ...
    print("\n" + "="*70)
    print("SEEDING DATABASE (ADD MISSING PEPTIDES)")
    print("="*70 + "\n")

    for peptide_data in combined:
        try:
            name = peptide_data.get("name")
            if not name:
                print("✗ Skipped entry with missing 'name'")
                errors += 1
                continue

            existing = db.get_peptide_by_name(name)
            if existing:
                skipped += 1
                continue

            db.add_peptide(**peptide_data)
            added += 1
        except Exception as e:
            errors += 1
            print(f"✗ Error adding {peptide_data.get('name','(unknown)')}: {e}")

    total_now = len(db.list_peptides())

    print("\n" + "-"*70)
    print(f"Seeding complete: added={added}, skipped_existing={skipped}, errors={errors}, total_now={total_now}")
    print("-"*70 + "\n")
